...
1. **字形不能只用阈值切，要用连续的覆盖率。**
   ...
   覆盖率的两端各削一小段（[NOISE_FLOOR] 和 [WHITE_CEIL]），因为黑侧和白侧
   都带压缩噪声：黑侧是 0~4 的块状痕迹、白侧是 251~253。不削的话，整块渐变会被
   掺进万分之一的白，白 R 上会浮出几个肉眼几乎看不见、但确实存在的彩色点。

2. **形状最外沿要先排除掉一圈，再判断字形。**

   针对的是这种情况：主体掩码的最外 1 像素上挂着一圈浅色描边（比如早先那版
   app_icon.png，亮度 150~211、alpha 常常是 255），那是裁剪时带进来的杂质、
   不属于设计。如果和字形一起按亮度判断，它会因为「够亮」而被当成字形保留成白色
   —— 结果就是渐变块的整个外沿镶上一圈白线。
   所以先按 [--trim] 把形状掩码往里收几像素，只在收进来的内部做字形判断，
   外圈一律当主体填渐变。

   这个值不需要调得很准：只要「大于杂质厚度（1~2px）、小于主体到字形的距离」，
   都是对的。当前这版 app_icon.png 是满幅黑底、外沿没有杂质，收掉的像素数是 0，
   等于没生效 —— 留着是因为换一张图它就有用。
...
3. **渐变轴归一化到形状，不是画布。**
   ...
   当前这版 app_icon.png 的不透明区就是整幅画布，于是「沿形状」和「沿画布」
   退化成同一件事 —— 正是想要的：整张图从左上角的纯紫推到右下角的纯青。
...
# 高于这个亮度的一律当纯字形（压掉白区上同样存在的压缩噪声）。
#
# 和 [NOISE_FLOOR] 是一对，必须一起有。只削黑侧会出现一个很别扭的后果：
# 白 R 的实心区域本身带 251~253 的压缩噪声，按 (lum-8)/255 算出来覆盖率是 98.4%，
# 于是这十几个像素会被掺进 1.6% 的渐变色 —— 白字上出现几个几乎看不见、
# 但确实存在的彩色点。把上界放到 248，白区就干净地吸附到纯白。
WHITE_CEIL = 248
...
def main():
    ...
    ap.add_argument("--white-ceil", type=int, default=WHITE_CEIL,
                    help="高于此亮度算纯字形，压掉白区噪声（默认 %d）" % WHITE_CEIL)
    ...
    cov = np.clip((lum - args.noise_floor) / float(args.white_ceil - args.noise_floor),
                  0.0, 1.0)
